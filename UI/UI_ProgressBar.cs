...
public class UI_ProgressBar : MonoBehaviour
{
    ...
    {
        //UpdateFillVisual(true);
    }

    /// <summary>
    /// Update the fill and text visuals
    /// </summary>
    protected void UpdateProgressVisuals()
    {
        UpdateFillVisual();
    ...
    /// <summary>
    /// Searches for an IHealth component starting with the GameObject itself,
    /// and then traversing up its parent hierarchy.
    /// </summary>
    /// <param name="startObject">The GameObject to start the search from.</param>
    /// <returns>The first found IHealth component or null if none is found.</returns>
    public IHealth FindHealthComponent(GameObject startObject)
    {
        Transform currentTransform = startObject.transform;

        while (currentTransform != null)
        {
            IHealth healthComponent = currentTransform.GetComponent<IHealth>();
    ...
    /// </summary>
    protected void UpdateFillVisual()
    {
        if (Fill == null)
        {
            return;
